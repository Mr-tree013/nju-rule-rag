...
NJU Rule RAG 是一个面向南京大学本科新生的**校规与校园生活智能问答系统**。基于 70 份公开校规、办事指南和校园生活文档，支持自然语言提问、来源引用、风险分级与拒答机制。提供 FastAPI 接口和 QQ 群机器人两种交互方式。
...
| 维度 | 数值 |
|------|------|
| 资料来源 | 51 条来源（70 份文档：校规 + 通知 + 校园生活指南） |
| 可检索片段 | 3771 chunks |
| 核心代码 | 25 个 Python 文件（app/ 11 + scripts/ 14） |
| 单元测试 | 110 个 |
| 评测问题 | 70 题 / 14 个主题 |
| Docker 支持 | ✅ 一条命令部署 |
| 多模型评测 | DeepSeek / Qwen3-8B / Baseline 对照 |
| 多向量模型 | text2vec-base-chinese / bge-m3 对照 |
...
│    │  annotate_gold_sources  check_regression       │
│    │  (110 个自动化测试)                              │
│    └────────────────────────────────────┘            │
└────────────────────────────────────────────────────┘
```
...
| `build_index.py` | chunks → BM25 + Chroma 向量索引，支持 text2vec / bge-m3 多模型 |
| `tune_weights.py` | 混合检索权重网格搜索调优 |
...
| `eval_retrieval.py` | 检索质量独立评测（多向量模型对照：text2vec / bge-m3 / baseline） |
| `eval_generation.py` | 生成质量独立评测（多模型对比：DeepSeek / Qwen3-8B） |
...
| 语义检索 | ChromaDB + text2vec-base-chinese（支持 bge-m3 等多模型切换） |
| 重排序 | app/reranker.py（检索结果二次打分） |
| 文本生成 | OpenAI 兼容接口（DeepSeek / Qwen / 智谱 / Ollama 本地模型） |
| 权重调优 | tune_weights.py（混合检索 BM25/Vector/Priority 权重网格搜索） |
| 风险控制 | policy.py（关键词分类 + 规则 + 来源审计） |
| 格式转换 | PyMuPDF + BeautifulSoup + LibreOffice + python-docx |
| QQ Bot | NapCatQQ + OneBot v11 HTTP 回调（QQ 小号方案） |
| 评测体系 | pytest (110 tests) + 四层评测（端到端 / 检索 / 生成 / 回归） |
