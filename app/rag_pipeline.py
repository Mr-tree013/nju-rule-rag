...
3. 如果资料片段不足以回答用户的问题，必须明确说："抱歉，目前没有找到与您问题相关的足够可靠的校规依据。建议联系所在院系教务员或辅导员获取准确信息。"
4. 对于涉及退学、开除、处分、作弊、学位、毕业资格等高风险问题，只提供校规中已有的客观规定描述，不得对用户个人情况做出判断或结论。
5. 回答要简洁、直接，控制在 200 字以内，适合在 QQ 群里阅读。
...
def answer_question(question):
    ...
    # 5. 高风险问题使用更严格的阈值
    if risk_level == "high":
        strong_chunks = [c for c in reliable_chunks if c["score"] >= 0.3]
